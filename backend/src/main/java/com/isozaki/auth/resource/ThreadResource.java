...
public class ThreadResource {
     ...
     * コメントは作成日時の降順で表示される。認証不要。</p>
     *
     * @param artistId アーティストID
     * @param threadId スレッドID
     * @param page     コメントページ番号（1始まり、デフォルト1）
     * @param size     1ページあたりのコメント件数（デフォルト10）
    ...
    public Response getThreadDetail(
            @PathParam("artistId") String artistId,
            @PathParam("threadId") int threadId,
            @QueryParam("page") @DefaultValue("1") int page,
            @QueryParam("size") @DefaultValue("10") int size) {

        Optional<ThreadDetailResponse> result = threadService.getThreadDetail(artistId, threadId, page, size);
     ...
     * @return 201 Created: 作成成功、401 Unauthorized: 認証失敗、404 Not Found: アーティスト不在
     */
    @POST
    public Response createThread(
        ...
        Optional<ThreadDetailResponse> result = threadService.createThread(artistId, request);
     ...
     * @return 201 Created: 追加成功、401 Unauthorized: 認証失敗、404 Not Found: スレッド不在
     */
    @POST
    ...
    public Response addComment(
        ...
        Optional<ThreadCommentResponse> result = threadService.addComment(artistId, threadId, request);
